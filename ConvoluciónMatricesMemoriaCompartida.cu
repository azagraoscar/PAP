...
{
	__shared__ int b_shared[WIDTH_B][WIDTH_B];			//Declaramos una matriz del mismo tamaño que B en la memoria compartida
	...
	if (threadIdx.x < WIDTH_B && threadIdx.y < WIDTH_B) //Hacemos que los hilos del 0,0 al 2,2 (9 hilos) se encarguen de cargar la matriz B en memoria compartida
	{
		b_shared[threadIdx.x][threadIdx.y] = b[threadIdx.y + threadIdx.x * WIDTH_B]; 
		__syncthreads();
	}

	//Ahora realizamos la convolución
	...
	{																		//tenemos que hacer la comprobación de que los hilos estén dentro de la matriz
		...
		for (int i = 0; i < WIDTH_B; i++)		//recorremos las filas de la matriz B
		{
			for (int j = 0; j < WIDTH_B; j++)	//recorremos las columnas de la matriz B
			{
				...
				{
					sum += a[pos_j + pos_i * WIDTH] * b_shared[i][j]; // Calculamos la convolución usando la matriz B en memoria compartida
					__syncthreads();
