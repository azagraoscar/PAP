...
{
	int pos = threadIdx.x + threadIdx.y * blockDim.y; //Calculamos la posición de R que vamos a obtener
	int sum = 0;
	...
	for (int i = 0; i < WIDTH_B; i++)		//recorremos las filas de la matriz B
	{
		...
		{
			pos_i = threadIdx.y + (i - (WIDTH_B - 1));  //calculamos la posición de la i restándole a la posición del hilo el desplazamiento de B 
			pos_j = threadIdx.x + (j - (WIDTH_B - 1));  //calculamos la posición de la j restándole a la posición del hilo el desplazamiento de B
			if (!(pos_i < 0 || pos_j < 0 || pos_i >= WIDTH || pos_j >= WIDTH))  //si ninguno de los índices es negativo o mayor que la anchura máxima significa que estamos dentro de la matriz A, y podemos por lo tanto operar
			{
...
int main()
{
	...
	//calculamos el tamaño del bloque. En este caso, ya que contamos con solo un bloque, el tamaño es el tamaño de la matriz resultado
	dim3 blockSize(width_res, width_res);
	...
	for (int i = 1; i <= width_res * width_res; i++)
	{
		h_res[i] = 0;
	}
	...
	convolucion << <1, blockSize >> > (d_res, d_a, d_bi);
